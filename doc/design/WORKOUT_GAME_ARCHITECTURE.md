...
- The current scene has bounded road, feature, and deterministic forest
  geometry, stable species/variant instance batches, and no unbounded particle
  list. Instance buffers are uploaded only when placement changes or a visible
  opacity step exceeds 0.025. Automatic frame-budget quality scaling remains a
  future enhancement.
...
| Near forest dressing | Thirty-six deterministic foreground trees in near/far rows plus 28 floor props and 14 trail-edge clusters generated by `WorkoutGameForestComposition`; 45-metre dry-pine, moist-spruce and rocky-mixed zones control bounded variant and scale ranges, while the view model anchors every item to the authoritative terrain profile. Four tree, sixteen floor-prop and six verge-cluster delegates remain stable; `WorkoutGameForestInstancing` changes their per-variant instance tables at resident boundaries and applies camera/visibility fades per instance. Forest props use one vertex-color material and primitive per authored variant so color variety does not multiply draw calls. |
...
6. **P2: the legacy comparison renderer still scales poorly with richer
   ...
   Quick 3D now retains unchanged chunks, forms changed resident geometry on a
   capacity-one worker, and batches the current forest catalog by vertex-colored
   instanced variant. Future authored catalogs must follow the same measured
   batching contract instead of adding one delegate or material per object.
...
from the newer snapshot instead of numerically interpolated. One-shot audio uses
a separate fixed-capacity event journal owned by the simulation. Each published
engine frame carries the eight newest events, their monotonic ids and a reset
epoch. The GUI consumes only unseen ids, so the latest-value mailbox may replace
intermediate frames without losing a feature or landing cue. A pause/resume
generation change does not alter the journal epoch or replay consumed sounds;
simulation reset and backward seek do. If the consumer falls behind by more
than eight events, the oldest entry is discarded without growing memory or
blocking simulation. The physical spacing of workout features and landings
keeps normal production traffic far below that bound. If only one valid
snapshot exists, the renderer displays it without extrapolating. Missing frames
reduce visual smoothness but cannot feed back into physics, scoring, trainer
control or recording.

Audio rendering is a GUI-only optional adapter. Its preference defaults off and
the two `QSoundEffect` resources are created lazily when enabled. Feature cues
are emitted once when a new action enters its measurement phase; landing cues
use the authoritative physics impact rising edge and bounded impact strength.
The assets are small project-authored PCM resources generated by the checked-in
FFmpeg recipe. Audio playback performs no per-frame file access and has no
dependency edge back to the engine, trainer controller or recorder.
