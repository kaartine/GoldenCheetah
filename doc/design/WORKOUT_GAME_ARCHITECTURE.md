...
| Near forest dressing | Eighteen deterministic foreground-tree slots plus fourteen bilateral floor/verge pairs generated by `WorkoutGameForestComposition`; 45-metre dry-pine, moist-spruce and rocky-mixed zones control bounded variant and scale ranges, while the view model anchors every item to the authoritative terrain profile. Stable identities remain resident between slot boundaries and QML fades camera-corridor and visibility-band crossings instead of deleting delegates mid-frame |
