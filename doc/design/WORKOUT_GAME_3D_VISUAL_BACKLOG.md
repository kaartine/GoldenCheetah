...
| VIS-19 | P0 | open | Workout Game visibly stutters during the first seconds after training starts even when steady-state rendering is smooth. | A cold-start motion capture from the packaged application shows continuous camera, rider and world movement from the first presented gameplay frame. The first ten seconds meet the dedicated 25 ms p99 and 50 ms maximum frame-time limits with no repeated late frames or hidden startup pause. |
