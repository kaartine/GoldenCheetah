...
| VIS-19 | P0 | open | Workout Game visibly stutters during the first seconds after training starts even when steady-state rendering is smooth. | A cold-start motion capture from the packaged application shows the first swap and first visual change within 100 ms, then continuous camera, rider and world movement. Swap intervals after the first presented frame meet the dedicated 25 ms p99 and 50 ms maximum limits, visual-revision stalls stay within 50 ms, and there are no repeated late frames or hidden startup pauses. |
