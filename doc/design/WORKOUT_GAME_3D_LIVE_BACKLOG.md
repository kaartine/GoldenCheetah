...
| --- | --- | --- | --- |
| LIVE-ROUTE-01 | in progress | Converted workouts contain long, visually straight sections. | A deterministic route-quality audit limits uninterrupted near-straight distance and includes gentle, medium and sharp rideable turns, including selected turns approaching 90 degrees. |
| LIVE-ROUTE-02 | in progress | Berms still behave like advertised features instead of ordinary trail bends. | Banked turns are part of normal route geometry, produce no feature prompt, and continuously move and lean the rider according to effort. |
...
| LIVE-ROUTE-06 | open | A converted or re-saved course such as `test3` can still feel like mostly straight road. | Conversion performs a deterministic quality pass before persistence and rejects a route without enough bends, switchbacks, rollers and elevation variation for its length. |
