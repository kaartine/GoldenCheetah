...
| LIVE-RIDER-09 | open | Successful MTB jumps need a speed-dependent tailwhip instead of a rigid airborne pose or a side flip. | After take-off, the rider keeps the front wheel and bars controlled while the bicycle rear rotates laterally around the steering/front-frame region. The normalized launch speed selects a bounded 0-35 degree peak tail angle: a low safe speed produces little or no whip and a fast jump produces the largest motion. The tail reaches its peak near mid-flight, returns smoothly before landing and touches down within 2 degrees of the authoritative travel direction. The animation never changes the physics root, selected line, collision result or recording, and is disabled on grounded bypasses. |
| LIVE-RIDER-10 | open | A synthetic large seek can rebuild the local terrain around the new rider position while the bounded camera remains near the old chunk, temporarily leaving only the clear colour visible. | A seek, workout restart or other authoritative distance discontinuity resets the camera and presentation filters atomically to the new road sample before publishing the rebuilt chunk. The first rendered frame keeps the rider, trail and terrain visible without weakening normal frame-to-frame camera speed limits. |
| LIVE-RIDER-11 | verified | Root surface relief previously reached the chase camera after its normal following delay and could feel like excessive repeated vibration. | The chase base follows root-free authored support terrain while the rider and suspension retain the physical root profile. A separate main-line cue is low-pass filtered, reaches about 5 mm on the catalog roots and is hard-limited to 22 mm and 0.18 m/s; the safe bypass remains still. Camera continuity tests pass at 30 and 60 FPS and the focused Qt Quick 3D test passes on NVIDIA. |
