...
CHART_FIXTURE_TEST_NAME ?= testChartOwnership
...
CHART_FIXTURE_LINK = $(LINK) $(LFLAGS) -o $(CHART_FIXTURE_BINARY) $(CHART_FIXTURE_OBJECTS) $(CHART_FIXTURE_OUT)/renamed-main.o $(CHART_FIXTURE_OUT)/$(CHART_FIXTURE_TEST_NAME).o $(OBJCOMP) $(CHART_FIXTURE_QT)/lib/libQt6Test.so $(CHART_FIXTURE_LIBS)

.PHONY: chart-ownership
chart-ownership:
	@test -n "$(CHART_FIXTURE_OUT)" && test -d "$(CHART_FIXTURE_OUT)"
	$(CHART_FIXTURE_QT)/libexec/moc $(DEFINES) $(INCPATH) -I$(CHART_FIXTURE_QT)/include/QtTest $(CHART_FIXTURE_SOURCE)/$(CHART_FIXTURE_TEST_NAME).cpp -o $(CHART_FIXTURE_OUT)/$(CHART_FIXTURE_TEST_NAME).moc
	$(CXX) -c $(CXXFLAGS) $(INCPATH) -DQT_TESTLIB_LIB -I$(CHART_FIXTURE_QT)/include/QtTest -I$(CHART_FIXTURE_OUT) $(CHART_FIXTURE_SOURCE)/$(CHART_FIXTURE_TEST_NAME).cpp -o $(CHART_FIXTURE_OUT)/$(CHART_FIXTURE_TEST_NAME).o
